(ns spariev.logsearch.bobo
  ...
  (:import (com.browseengine.bobo.facets.impl SimpleFacetHandler)
	   ...
	   (com.browseengine.bobo.api BrowseRequest BrowseSelection BrowseException BrowseResult
				      BrowseFacet BoboIndexReader Browsable BoboBrowser BrowseHit)))
...
(defn browse
  ...
  (let [facets (facet-handlers attrs-values)
	bobo-reader (BoboIndexReader/getInstance index-reader (java.util.ArrayList. facets))
	lucene-query          (build-query query)
	browse-selections (map #(doto (BrowseSelection. %)
				  (.addValue (attrs-values %))) (keys attrs-values))
	...
	hits (seq (.getHits browse-result))]
